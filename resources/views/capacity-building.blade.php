...
<p class="mb-10">Over the last 10 years of supporting CCRP grantees, we have created many different resources, including videos, reference documents, training guides and webinar recordings. You can find these on our <a href="{{ config('app.resources_site_url') }}/resources?refinementList%5Baudiences.name.en%5D%5B0%5D=CCRP"> resources repository</a> on the Stats4SD website.
...
<div class="card  lg:w-96 bg-base-100 shadow-xl image-full mx-3 mb-5 ">
  <figure><img src="{{ $trove->trove_cover_image }}" alt="{{ $trove->trove_data['title'] }} . 'cover image'" class="" /></figure>
  <div class="card-body  self-end mt-10">
    <h3 class="card-title">{{ $trove->trove_data['title'] }}</h3>
    <p>{!! Str::limit($trove->trove_data['description'], 200, '...') !!}</p>

  </div>
  <a class="block absolute h-full w-full top-0 z-50 rounded-2xl hover:bg-black hover:opacity-20" href="{{ config('app.resources_site_url') . '/resources/' . $trove->trove_data['slug'] }}" target="blank"> </a>
</div>
...
<div class="carousel w-full 2xl:w-5/6 mt-4 mb-10 max-h-80 md:max-h-full mx-auto drop-shadow-xl" style="height: 420px">
  ...
  <a href="{{ config('app.resources_site_url')}}/resources?refinementList%5Baudiences.name.en%5D%5B0%5D=CCRP"><button class="btn-primary block mx-auto">View all RMS Resources</button></a>
